...
class ReadSQLHandler(BaseStageHandler):
    ...
    async def _handle_need_write_or_email(self, memory: Memory, user_input: str) -> StageHandlerResult:
        ...
        if wants_write:
            return await self._handle_write_data(memory, user_input)
        elif wants_email:
            return await self._handle_send_email(memory, user_input)
    ...
    async def _handle_write_data(self, memory: Memory, user_input: str) -> StageHandlerResult:
        """Handle write_data flow."""
    ...
    async def _fetch_schemas_for_result(self, memory: Memory, connection_name: str) -> StageHandlerResult:
        """Fetch schemas for result connection (read_sql with execute_query)."""
        logger.info(f"📋 Fetching schemas for result connection: {connection_name}")
    ...
    async def _fetch_schemas_for_write(self, memory: Memory, connection_name: str) -> StageHandlerResult:
        """Fetch schemas for a connection (write_data)."""
    ...
    async def _handle_send_email(self, memory: Memory, user_input: str) -> StageHandlerResult:
        """Handle send_email flow."""
