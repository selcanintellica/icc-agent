        ...
        if wants_write:
            ...
            if action.get("action") == "TOOL" and action.get("tool_name") == "send_email":
                ...
                if not memory.output_table_info:
                    ...
                    logger.warning("⚠️ No output_table_info - cannot send email without writing data first")
                    memory.gathered_params = {}
                    memory.current_tool = None
                    memory.last_question = None
                    return memory, "⚠️ You need to write the data to a table first before sending an email.\n\nPlease use 'write' to save the ReadSQL results to a table, then you can send an email.\n\nWhat would you like to do? (write / done)"
                ...
                memory.stage = Stage.CONFIRM_EMAIL_QUERY
                memory.email_query_confirmed = False
                
                return memory, f"📧 I will use this SQL query to fetch data from the result table:\n```sql\n{auto_query}\n```\nIs this correct? (yes/no)"
    ...
    if memory.stage == Stage.CONFIRM_EMAIL_QUERY:
        ...
        if "yes" in user_lower or "ok" in user_lower or "correct" in user_lower:
            ...
            try:
                ...
                return memory, f"✅ Email job created! Results will be sent to {params.get('to')}!\nAnything else? (write / email / done)"
            ...
            memory.stage = Stage.NEED_EMAIL_QUERY
            return memory, "Please provide the SQL query you want to use for the email:"
        
        else:
            return memory, "Please confirm: Say 'yes' to use this query or 'no' to provide a different one."
    
    # ========== STAGE: NEED_EMAIL_QUERY ==========
    if memory.stage == Stage.NEED_EMAIL_QUERY:
        # User provides their own SQL query
        user_query = user_utterance.strip()
        ...
        try:
            ...
            request = SendEmailLLMRequest(
                ...
                variables=[SendEmailVariables(
                    query=user_query,  # Use user-provided query
                    connection=connection_id,
                    to=params.get("to"),
                    subject=params.get("subject", "Query Results"),
                    text=params.get("text", "Please find the query results attached."),
            ...
            return memory, f"✅ Email job created with your custom query! Results will be sent to {params.get('to')}!\nAnything else? (write / email / done)"
